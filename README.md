...
- **A nota de pronúncia mede reconhecimento, não fonética.** Quem transcreve é o `SpeechRecognition` do navegador, e o trabalho dele é entender o aluno APESAR do sotaque — o motor tem modelo de linguagem e puxa para o inglês plausível. Na prática: "I fink so" tem boa chance de virar "I think so" e tirar nota alta. Custo zero em troca de perdoar demais. Nota de fonema exigiria serviço dedicado (Azure Speech e afins); feedback do tipo "põe a língua entre os dentes" exigiria um LLM ouvindo o áudio.
- **O feedback de pronúncia só diz QUAIS palavras não foram reconhecidas**, nunca qual som corrigir — é o teto do que dá para extrair de uma transcrição.
- **`pronuncia` não funciona no Firefox** — sem `SpeechRecognition`. A tela avisa e pede Chrome, Edge ou Safari, mas o aluno fica sem como responder aquela questão, o que hoje trava a conclusão da tarefa. Precisa de decisão de produto (pular? não gerar o tipo?) antes de ir para aluno real.
- **O áudio de `ordenar_audio` é gerado uma vez por questão, ao salvar** — TTS do Gemini (mesma chave e endpoint `generateContent` de `gerar-atividade`, só com `responseModalities: ["AUDIO"]`), guardado em `audio-questoes` e servido por URL assinada. O prompt já limita a 2 questões desse tipo por atividade, então o custo por atividade é pequeno e não recorrente — diferente da pronúncia, que seria por resposta se ainda usasse IA. Se a geração falhar ao salvar (rede, Gemini fora do ar), a questão fica sem áudio e a ficha da atividade mostra um aviso com botão para gerar de novo (`useGerarAudioAtividade`).
- **Chegou a ser `speechSynthesis` do navegador por um dia** (26/07/2026) — revertido depois de constatar em teste real que boa parte dos aparelhos não tem voz em inglês instalada, o que tornava o exercício impossível de fazer. TTS pré-gerado resolve isso ao custo de uma chamada paga por questão salva, não por aluno.
