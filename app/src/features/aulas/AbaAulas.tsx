      ...
      {aulas && aulas.length === 0 && (
        <div className="mt-4 rounded-3xl border-2 border-dashed border-neutral-300 px-6 py-12 text-center">
          ...
          <p className="mt-1 text-sm text-neutral-500">
            Registre a primeira aula de {alunoNome.split(" ")[0]}.
          </p>
        </div>
      )}
          ...
          {aulas.map((aula) => (
            <div key={aula.id} className="rounded-2xl bg-white p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-bold capitalize text-neutral-800">
                  ...
                  {aula.serie_id && (
                    <span className="mt-1.5 inline-flex items-center gap-1 rounded-full bg-violet-50 px-2 py-0.5 text-[10px] font-bold text-violet-700">
                      ...
                      {rotuloRecorrencia(aula.data_hora)}
                    </span>
                  )}
                </div>
...
export function ModalAula({
  ...
  const totalFuturas = aula
    ? (aulasDaSerie ?? []).filter(
        (a) => new Date(a.data_hora) >= new Date(aula.data_hora)
      ).length
    : 0;
  const afetadas =
    escopo === "todas" ? totalNaSerie : escopo === "futuras" ? totalFuturas : 1;

  const emAndamento =
    criar.isPending || atualizar.isPending || excluir.isPending;
  const erro =
    ...
    >
      ...
      >
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="block">
            ...
            {/* Passo de 1: aula de 50, 45 ou 20 min é comum e o passo de 15
                barrava o envio do formulário. O teto de 1024 (um dia) não é
        ...
        <label className="mt-3 block">
          <span className="text-xs font-bold text-neutral-600">
            Anotação (o que foi dado, o que revisar)
          </span>
          <textarea
            value={anotacao}
        ...
        {!editando && (
          <label className="mt-3 block">
            <span className="text-xs font-bold text-neutral-600">
              Repetir semanalmente (semanas extras)
            </span>
            <input
              type="number"
              min={0}
        ...
        {editando && emSerie && (
          <div className="mt-4 rounded-2xl bg-violet-50 p-3.5">
            <p className="flex items-center gap-1.5 text-xs font-extrabold text-violet-900">
              <Repeat className="h-3.5 w-3.5 shrink-0" /> Aula em série ·{" "}
              {rotuloRecorrencia(aula!.data_hora)}
            </p>
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={
                        marcado
                          ? "ml-auto text-violet-200"
                          : "ml-auto text-violet-400"
                      }
                    >
        ...
        {confirmandoExclusao && (
          <div className="mt-4 rounded-2xl bg-rose-50 p-3.5">
            <p className="flex items-center gap-1.5 text-xs font-extrabold text-rose-800">
              ...
              Excluir {afetadas} {afetadas === 1 ? "aula" : "aulas"} desta
              série?
            </p>
